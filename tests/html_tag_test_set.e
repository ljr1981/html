...
feature -- Test routines
	...
	html_tag_basic_tests
		...
		do
			create l_html
			assert_strings_equal ("html_page1", "<!DOCTYPE html><html><head/><body/></html>", l_html.html_out)

			create l_base
			l_html.head.extend (l_base)
			assert_strings_equal ("html_page2", "<!DOCTYPE html><html><head><base/></head><body/></html>", l_html.html_out)
			create l_link
			l_html.head.extend (l_link)
			assert_strings_equal ("html_page3", "<!DOCTYPE html><html><head><base/><link/></head><body/></html>", l_html.html_out)
			create l_meta
...
feature -- Testing: Creation Tests
	...
	general_html_tag_creation_tests
		...
		do
			...
			assert_strings_equal ("h1_h2_h3_1", "<h1><h2><h3/></h2></h1>", l_h1.html_out)

			create l_h1.make_with_content (<<create {HTML_H2}.make_with_content (<<create {HTML_H3}>>)>>)
			assert_strings_equal ("h1_h2_h3_2", "<h1><h2><h3/></h2></h1>", l_h1.html_out)

			create l_page.make_with_content (<<l_a, l_br, l_div, l_footer, l_form, l_h1>>)
			assert_strings_equal ("page", "<!DOCTYPE html><html><head/><body><a/><br/><div/><footer/><form/><h1><h2><h3/></h2></h1></body></html>", l_page.html_out)
		end
...
feature -- Testing: {HTML_HEADLINE_X}
	...
	html_headline_x_test
		...
		do
			create l_tag
			assert_strings_equal ("hx", "<h1/>", l_tag.new_hx.html_out)
			l_tag.new_hx.set_h1
			l_tag.new_hx.set_h2
			l_tag.new_hx.set_h3
			l_tag.new_hx.set_h4
